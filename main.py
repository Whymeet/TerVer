...
def calculate_statistics(df):
    ...
    S_group = str(round(V_vz_plus / V_n_plus * 100, 3)) + "%"  # Процент связанных вершин относительно общего числа связанных вершин
    Э_group = str(round(V_n_plus / N, 3)) + "%"  # Среднее число связей для каждой вершины
    BB_group = str(round((100 * V_vz_plus) / (0.5 * N * (N - 1)), 3)) + "%"  # Плотность связей в графе
